...
@Component({
  selector: 'app-variable-pay-earning-report',
  templateUrl: './variable-pay-earning-report.component.html',
  styleUrls: ['./variable-pay-earning-report.component.css']
})
export class VariablePayEarningReportComponent {
  currentDate: any;
    ...
    if (!this.includeEmployeeDetails) {
      ...
      for (let idx = 0; idx < this.data.length; idx++) {
        ...
        let obj = {
          ...
          'Gross Wages': this.data[idx].grossearning,
          'EPF Wages': this.data[idx].epf_wages,
          'EPS Wages': this.data[idx].eps_wages,
          'VPF': this.data[idx].vpf,
          'EDLI Wages': this.data[idx].edli_wages,
          'EPF Contri Remitted': this.data[idx].epf_contri_remitted,
          'EPS Contri Remitted': this.data[idx].eps_contri_remitted
      ...
      for (let idx = 0; idx < this.data.length; idx++) {
        ...
        let obj = {
          ...
          'Gross Wage': this.data[idx].grossearning,
          'EPF Wages': this.data[idx].epf_wages,
          'EPS Wages': this.data[idx].eps_wages,
          'EDLI Wages': this.data[idx].edli_wages,
          'EPF Contri Remitted': this.data[idx].epf_contri_remitted,
          'EPS Contri Remitted': this.data[idx].eps_contri_remitted,
          'EPF EPS Diff Remitted': this.data[idx].epf_eps_diff_remitted,
        ...
        if (!this.includeEmployeeDetails) {
          for (let idx = 0; idx < this.data.length; idx++) {
            let obj = [
              ...
              this.data[idx].grossearning,
              this.data[idx].epf_wages,
              this.data[idx].eps_wages,
              this.data[idx].vpf,
              this.data[idx].edli_wages,
              this.data[idx].epf_contri_remitted,
              this.data[idx].eps_contri_remitted
            ];
          ...
          for (let idx = 0; idx < this.data.length; idx++) {
            let obj = [
              ...
              this.data[idx].epf_contri_remitted,
              this.data[idx].eps_contri_remitted,
              this.data[idx].epf_eps_diff_remitted,
              this.data[idx].ncp_days,
              this.data[idx].refund_of_advances,
