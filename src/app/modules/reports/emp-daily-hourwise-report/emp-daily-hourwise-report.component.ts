...
@Component({
  selector: 'app-emp-daily-hourwise-report',
  templateUrl: './emp-daily-hourwise-report.component.html',
  styleUrls: ['./emp-daily-hourwise-report.component.css']
})
export class EmpDailyHourwiseReportComponent {
  ...
  constructor(
    ...
    if (this.router.getCurrentNavigation().extras.state != null || this.router.getCurrentNavigation().extras.state != undefined) {
      ...
      this.emp_name = this.router.getCurrentNavigation().extras.state.emp_name;
    }
  }
  ...
  get_hourly_summary_report() {
    ...
    this._ReportService.get_hourly_summary_report({
      ...
      "fromDate": from_date,
      "toDate": to_date,
      "emp_code": this.emp_code.toString(),
  ...
  exportToExcelTable() {
    ...
    XLSX.writeFile(workbook, 'Employee_Daily_Hour_Report.xlsx');
  }
