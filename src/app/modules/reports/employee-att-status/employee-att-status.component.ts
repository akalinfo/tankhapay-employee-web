...
@Component({
  selector: 'app-employee-att-status',
  templateUrl: './employee-att-status.component.html',
  styleUrls: ['./employee-att-status.component.css']
})
export class EmployeeAttStatusComponent {
      ...
      .subscribe((resData: any) => {
        if (resData.statusCode) {
          ...
          this.emp_json_data = (decrypted_emp_json_data).data.attendancedetail.filter((el: any) => {
            // return el.time_criteria === 'Full Time' && el.lockstatus === 'Locked'
            return true;
          })
  ...
  getAttExcel() {
    this._attendanceService.get_month_dates_days({
      ...
      if (resData.status) {
        ...
        //   // if (this.filteredEmployees[idx]?.lockstatus == 'Locked') {
        //   let leave_balance = '';
        ...
        //     'Status': (this.filteredEmployees[idx].monthly_att_approval_status == 'Y' ? 'Verified' :
        //       (this.filteredEmployees[idx].monthly_att_approval_status != 'Y' && (this.filteredEmployees[idx].present_days) >
        ...
        downloadLink.download = `Employee_Present_Absent_Status_${formattedDate}.xlsx`;
        downloadLink.click();
