...
@Component({
  selector: 'app-empl-investment',
  templateUrl: './empl-investment.component.html',
  styleUrls: ['./empl-investment.component.css'],
  animations: [grooveState, dongleState]
})
export class EmplInvestmentComponent {
  [x: string]: any;
  orgempCode: any;
  ...
  previous_Employer_Popup: boolean = false;
  SavePreviousIncomeDetail_Form: FormGroup;
  previousSalaryDetails_data: any = [];
  previosSalaryHeaderName: any = '';
  ...
  HRADeclaration_Popup: boolean = false;
  HRADeclaration_Form: FormGroup;
  ...
  ngOnInit() {
    ...
    this.SavePreviousIncomeDetail_Form = this._formBuilder.group({
      orgEmpCode: ['', Validators.required],
      empName: ['', Validators.required],
      grossEarning: ['', Validators.required],
    ...
    this.HRADeclaration_Form = this._formBuilder.group({
      headId: ['', [Validators.required]],
      enc_empCode: ['', [Validators.required]],
      tenureId: ['', [Validators.required]],
      fromDate: ['', [Validators.required]],
  ...
  checkInvestmentDeclarationStatus(message: string): boolean {
    // Regular expression to extract dates in DD/MM/YYYY format
    const dateRegex = /(\d{2}\/\d{2}\/\d{4})/g;
    const dates = message.match(dateRegex);
  
    if (dates && dates.length === 2) {
      const fromDateStr = dates[0]; // 01/01/2025
      const toDateStr = dates[1];   // 28/02/2025
      ...
      // Check if 'toDate' is greater than or equal to today's date
      if (toDate >= currentDate && fromDate <= currentDate) {
        return true; // Investment declaration is open
      } else {
        return false; // Investment declaration is closed
      }
    } else {
  ...
  isNoDataOrAllApproved(): boolean {
    return this.filtered_data.length === 0 || this.filtered_data.every(report => report.status !== 'pending');
  }

  GetTaxProjection(taxByMonth_data: any) {
    // console.log(taxByMonth_data);
    // return;
    ...
    this._ReportService.GetTaxProjectionApi({
      ...
      if (resData.statusCode) {
        ...
        this.previousSalaryDetails_data = this.tax_data?.previousSalaryDetails[0];
        ...
        this.previousSalaryDetails_data = [];
        this.show_label = false;
      }
  ...
  updateRegimeStatus() {
    ...
    const confirmChange = confirm(`Are you sure, you want to Submit?`);
    if (confirmChange) {
  ...
  Save80CInvestmentProof() {
    let formData = this.UploadInvestmentProof_Form.value;
    // Check max_limit only if it has a valid number (not empty)
  ...
  SaveHomeLoanDocuments() {
    ...
    if (this.UploadHomeLoan_Form.valid) {
      ...
      this._EmployeeManagementService.saveHomeLoanDetails({
        ...
        "homeAddress": formData?.homeAddress.toString(),
  ...
  PreviousIncomeSavePopup(taxByMonth_data: any) {
    this.previosSalaryHeaderName = 'Save';
    this.SavePreviousIncomeDetail_Form.patchValue({
      orgEmpCode: taxByMonth_data?.orgempcode,
      empName: taxByMonth_data?.emp_name

    });
    this.previous_Employer_Popup = true;
  }
  closePreviousIncome() {
    this.previous_Employer_Popup = false;
    this.submitted = false;
    this.previosSalaryHeaderName = '';
    this.SavePreviousIncomeDetail_Form.reset();
  }

  SaveEditPreviousIncomeDetail(taxByMonth_data: any) {
    this.submitted = true;
    if (this.SavePreviousIncomeDetail_Form.valid) {
      let formData = this.SavePreviousIncomeDetail_Form.value;
      this._EmployeeManagementService.SavePreviousIncomeDetail({
        customerAccountId: this._EncrypterService.aesEncrypt(this.tp_account_id?.toString()),
        orgEmpCode: this._EncrypterService.aesEncrypt(formData?.orgEmpCode.toString()),
        empName: formData?.empName.toString(),
        finYear: this.bankDetailsForm.get('financialyear')?.value,
        grossEarning: formData?.grossEarning.toString(),
        basic: formData?.basic.toString(),
        hra: formData?.hra && formData?.hra.trim() !== '' ? formData?.hra.toString() : "0",
        tds: formData?.tds && formData?.tds.trim() !== '' ? formData?.tds.toString() : "0",
        pf: formData?.pf && formData?.pf.trim() !== '' ? formData?.pf.toString() : "0",
        vpf: formData?.vpf && formData?.vpf.trim() !== '' ? formData?.vpf.toString() : "0",
        insurance: formData?.insurance && formData?.insurance.trim() !== '' ? formData?.insurance.toString() : "0",
        createdBy: this.tp_account_id?.toString(),
        createdByUsername: this.employer_name.toString(),
        productTypeId: this._EncrypterService.aesEncrypt(this.product_type.toString()),
      }).subscribe((resData: any) => {
        if (resData.statusCode) {
          this.toastr.success(resData.message, 'Success');
        ...
        this.previous_Employer_Popup = false;
        this.previosSalaryHeaderName = '';
        this.GetTaxProjection(taxByMonth_data)
      });
    }

  }

  PreviousIncomeEditPopup(previousSalary_data: any) {
    this.previosSalaryHeaderName = 'Update';
    this.SavePreviousIncomeDetail_Form.patchValue({
      orgEmpCode: previousSalary_data?.orgempcode,
      empName: previousSalary_data?.empname,
      grossEarning: previousSalary_data?.grossearning.toString(),
      basic: previousSalary_data?.basic.toString(),
      hra: previousSalary_data?.hra.toString(),
      tds: previousSalary_data?.tds.toString(),
      pf: previousSalary_data?.pf.toString(),
      vpf: previousSalary_data?.vpf.toString(),
      insurance: previousSalary_data?.insurance.toString(),
    });
    this.previous_Employer_Popup = true;
  ...
  GetEmpRegimeAndTaxProjection(taxByMonth_data: any) {
    ...
    this._EmployeeManagementService.GetEmpRegimeAndTaxProjection({
      ...
      if (resData.statusCode) {
        ...
        this.isopenDeclarationProof = this.checkInvestmentDeclarationStatus(commonData.regimeDetails.declarationmessage);
  ...
  this._ReportService.GetTaxProjectionApi({
    ...
    if (resData.statusCode) {
      ...
      if (this.taxByMonth_data && this.taxByMonth_data.fields) {
        ...
        htmlBody += `
          <tr>
            <td>${this.taxByMonth_data.fields}</td>
            ${months.map(month => `<td>${this.taxByMonth_data[month] || '0'}</td>`).join('')}
            <td>${this.taxByMonth_data.totalgrossearning || '0'}</td>
          </tr>
        `;
      } else {
