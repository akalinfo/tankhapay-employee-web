...
@Component({
  selector: 'app-reporting-contract-renewal',
  templateUrl: './reporting-contract-renewal.component.html',
  styleUrls: ['./reporting-contract-renewal.component.css']
})
export class ReportingContractRenewalComponent {
  ...
  ngOnInit() {
    ...
    this.myForm = this._formBuilder.group({
      ...
      contractRenewal: ['', Validators.required],
      promotion: ['', Validators.required],
      roleChange: ['', Validators.required],
      roleBrief: ['']
    });
  }
  ...
  get_employee_list() {
    ...
    this._loginService.get_tpay_dashboard_data({
      "action": "get_contract_renewal_fromreporting",
  ...
  sendtoreporting() {
    ...
    const contractRenewal = this.myForm.get('contractRenewal');
    const promotion = this.myForm.get('promotion');
    const roleChange = this.myForm.get('roleChange');
    const roleBrief = this.myForm.get('roleBrief');
    // Validation
    if (!description?.value || description.invalid) {
      this.toastr.error('Description is required.');
      return;
    }

    if (!contractRenewal?.value || contractRenewal.invalid) {
      this.toastr.error('Contract Renewal selection is required.');
      return;
    }

    if (!promotion?.value || promotion.invalid) {
      this.toastr.error('Promotion selection is required.');
      return;
    }

    if (!roleChange?.value || roleChange.invalid) {
      this.toastr.error('Role Change selection is required.');
      return;
    }

    if (roleChange.value === 'Yes' && (!roleBrief?.value || roleBrief.invalid)) {
      this.toastr.error('Please enter brief of new role and responsibility.');
      return;
    }
    if (this.selectedEmployee) {
      ...
      let postData = {
        ...
        contractRenewal: contractRenewal.value,
        promotion: promotion.value,
        roleChange: roleChange.value,
        roleBrief: roleBrief?.value || '',
