...
@Component({
  selector: 'app-reporting-probation-renewal',
  templateUrl: './reporting-probation-renewal.component.html',
  styleUrls: ['./reporting-probation-renewal.component.css'],
  providers: [DatePipe]
})
export class ReportingProbationRenewalComponent {
  ...
  ngOnInit() {
    ...
    this.myForm = this._formBuilder.group({
      ...
      statusAction: ['', Validators.required],
      extendDate: ['', Validators.required]
    });
  ...
  get_employee_list() {
    ...
    this._loginService.get_tpay_dashboard_data({
      "action": "get_probation_renewal_fromreporting",
  ...
  alertsendforrenewal(employee: any) {
    ...
    const now = new Date();
    const istDate = this.datePipe.transform(now, 'yyyy-MM-dd', 'Asia/Kolkata');
    console.log(istDate)
    if (istDate) {
      this.myForm.patchValue({ extendDate: istDate });
    }
    //this.existingDate = employee.probation_end_date;
    // if (this.existingDate) {
    //   this.myForm.patchValue({ extendDate: this.existingDate });
    // }
  ...
  sendtoreporting() {
    ...
    const statusActionControl = this.myForm.get('statusAction');
    const extendDateControl = this.myForm.get('extendDate');
    ...
    // First, validate statusActionControl
    if (statusActionControl?.invalid) {
      if (statusActionControl.hasError('required')) {
        this.toastr.error('Approval Status is required.');
        return;
      }
    }

    // Then, validate extendDateControl
    // if(statusActionControl?.value === 'Extend'){
      if (extendDateControl?.invalid) {
        if (extendDateControl.hasError('required')) {
          this.toastr.error('Date is required.');
          return;
        }
      }

      const extendDate = new Date(extendDateControl?.value);
      // const existingDate = new Date(this.existingDate);
      formattedDate = this.formatDateToDDMMYYYY(extendDate);
          
      // if(statusActionControl?.value === 'Extend'){
      //   if (extendDate <= existingDate) {
      //     this.toastr.error('Extend Date must be later than the existing date.');
      //     return;
      //   }
      // }
    // }
    ...
    let postData = {
      ...
      emp_id: this.selectedEmployee.emp_id.toString(), //'701',
      renewal_of_emp_code: this.selectedEmployee.empcodeforrenewal.toString(),
      status: 'senttohr',
      description: this.myForm.get('description').value,
      approval_status: this.myForm.get('statusAction').value,
      et_date: formattedDate,
  ...
  onStatusChange(): void {
    const status = this.myForm.get('statusAction')?.value;
    console.log('Status changed to:', status);
    // if (status === 'Extend' && this.isSubmitted) {
    //   this.showValidationToasts();
    // }
  }
