...
@Component({
  selector: 'app-reimbursement-payment',
  templateUrl: './reimbursement-payment.component.html',
  styleUrls: ['./reimbursement-payment.component.css'],
  animations: [dongleState, grooveState],
})
export class ReimbursementPaymentComponent {
  ...
  Generate_Required_AmountPI() {
    this._PayoutService.GenerateRequiredAmountPI({
      ...
      "isBillable": 'N',
      "numberOfEmployees": this.Checkbox_Count?.toString(),
      "source": "Web",
  ...
  CalcReciebaleFromBaseAmount() {
    this._PayoutService.CalcReciebaleFromBaseAmount({
      "encrypted": this._EncrypterService.aesEncrypt(JSON.stringify({
        ...
        "baseAmount": this.amount?.toString(),
        "numberOfEmployees": this.Checkbox_Count?.toString(),
        "productTypeId": this.product_type,
  ...
  //       "numberofemployees": this.Checkbox_Count,
