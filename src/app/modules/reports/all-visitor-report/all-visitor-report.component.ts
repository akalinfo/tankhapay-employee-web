...
@Component({
  selector: 'app-all-visitor-report',
  templateUrl: './all-visitor-report.component.html',
  styleUrls: ['./all-visitor-report.component.css']
})
export class AllVisitorReportComponent {
  ...
  search(key: any) {
    ...
    this.filteredEmployees = this.visitor_list_data.filter(function (element: any) {
      ...
      element.assign_visiting_card1?.toString().toLowerCase().includes(invKey.toLowerCase())
      ...
      if (resData.status) {
        ...
        for (let idx = 0; idx < this.data.length; idx++) {
          let obj = {
            ...
            'Registration Date': this.formatDate(this.data[idx].createdon),
            'Last Check In Time':this.data[idx].check_in_time,
            'Last Check Out Time':this.data[idx].check_out_time,
            'Card Number': this.data[idx].assign_visiting_card1,
          }
          exportData.push(obj);
        }
        ...
        let date = new Date()
        downloadLink.download = 'All_Visitor_Report_' + this.employer_name.replaceAll(' ', '_').trim() + "_" + this.currentDateString.trim().replaceAll(' ', '_').trim() + '.xlsx';
        downloadLink.click();
      }
    })
  }
