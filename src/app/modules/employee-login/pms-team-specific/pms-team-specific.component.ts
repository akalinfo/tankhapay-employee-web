...
@Component({
  selector: 'app-pms-team-specific',
  templateUrl: './pms-team-specific.component.html',
  styleUrls: ['./pms-team-specific.component.css']
})
export class PmsTeamSpecificComponent {
  websiteUrl: any;
 ...
 let session_obj: any = JSON.parse(this._sessionService.get_user_session());
    ...
    const pmsts = CryptoJS.MD5('pmsts');
    
    console.log('harsh',"pmsts",pmsts);
    console.log('harsh',"hashedAccountId",hashedAccountId);
    console.log('harsh',"hash",hash);

    this.websiteUrl = this.sanitizer.bypassSecurityTrustResourceUrl(`${environment.PMS_TND_ATL_URL}/employee-sso/${hashedAccountId}/${hash}/${pmsts}`);
