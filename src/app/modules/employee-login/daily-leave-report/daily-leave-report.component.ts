...
export class DailyLeaveReportComponent {
  ...
  get_Leave_appl_by_account() {
    console.log(this.decoded_token.employee_flag.emp_id);
    console.log(this.decoded_token);

    this._ReportService.get_Leave_appl_by_account_empid({
      'action': 'get_Leave_appl_by_account_el',
      ...
      'emp_id': this.decoded_token.employee_flag.emp_id.toString()
    }).subscribe({
      next: (resData: any) => {
      ...
      if (resData.statusCode) {
        ...
        for (let idx = 0; idx < this.data.length; idx++) {
          let obj = {
            ...
            'Employee Id': (this.data[idx].orgempcode !== '' && this.data[idx].orgempcode != null && this.data[idx].orgempcode != undefined) ? this.data[idx].orgempcode : this.data[idx].tpcode,
            'Leave type': this.data[idx].leavetypename,
            'Type': this.data[idx].paid_unpaid,
            'Reason': this.data[idx].leave_description,
            'From date': this.data[idx].fromdate,
            'To date': this.data[idx].todate,
            'Leave Days': this.data[idx].leave_days,
            'Status': this.data[idx].approval_status,
          }
          exportData.push(obj);
        }
        ...
        let date = new Date()
        downloadLink.download = 'Daily_Leave_Report_' + this.employer_name.replaceAll(' ', '_').trim() + "_" + this.currentDateString.trim().replaceAll(' ', '_').trim() + '.xlsx';
        downloadLink.click();
      }
    })
  }
