...
@Component({
  selector: 'app-monthly-check-in-out-report',
  templateUrl: './monthly-check-in-out-report.component.html',
  styleUrls: ['./monthly-check-in-out-report.component.css']
})
export class MonthlyCheckInOutReportComponent {
    ...
    this._attendanceService.getEmployerMonthAttendance_for_excel({
      ...
      next: (resData: any) => {
        if (resData.statusCode) {
          ...
          let filteredData = download_excel_data_temp.filter((element: any) => {
            return (
              (element.emp_name.toLowerCase().includes(searchkey) ||
                element.orgempcode.toLowerCase().includes(searchkey) ||
                element.tpcode.toLowerCase().includes(searchkey) ||
                element.mobile.toLowerCase().includes(searchkey)) &&
              element.approval_status?.toLowerCase().includes(att_status_filter)
          ...
          const selectedEmps = this.filtered_employee_att_table_data.filter((e: any) => e.isChecked);
          if (selectedEmps.length > 0) {
            this.download_excel_data = filteredData.filter((element: any) => {
              return selectedEmps.some(sel => sel.emp_id === element.emp_id);
            });
          } else {
            this.download_excel_data = filteredData;
          }
      ...
      if (resData.status) {
        ...
        for (let idx = 0; idx < this.download_excel_data.length; idx++) {
          let obj = {
            ...
            'Email': this.download_excel_data[idx].email,
            'Mobile': this.download_excel_data[idx].mobile,
            'OrganizationUnit ': this.download_excel_data[idx].assignedous,
            'Vendor Name': this.download_excel_data[idx].vendor_name,
            'Project Name': this.download_excel_data[idx].project_name,
            'Salary Book Project': this.download_excel_data[idx].salary_book_project,
          ...
          for (let i = 0; i < resData.commonData.length; i++) {
            ...
            if (this.showGermanTime == 'IST') {
              obj[dateColumn] = attendanceType;

            } else if (this.showGermanTime == 'CET') {
              const parts = attendanceType.split('|');
              if (parts.length == 1) {
                obj[dateColumn] = attendanceType;

              } else if (parts.length >= 2) {
                let temp = parts[0] + '|' + this.istToGermanTimeService.convertIstToGermanTime(parts[1].split('-')[0], dateColumn)

                if (parts[1].split('-').length > 1) {
                  temp += '-' + this.istToGermanTimeService.convertIstToGermanTime(parts[1].split('-')[1], dateColumn);
                }
        ...
        downloadLink.download = `Monthly_CheckIn_Out_Report_${formattedDate}.xlsx`;
        downloadLink.click();

        this.filtered_employee_att_table_data.forEach(emp => emp.isChecked = false);
  ...
  checkIfAllSelected() {
    this.masterSelected = this.filtered_employee_att_table_data.every(emp => emp.isChecked);
  }
